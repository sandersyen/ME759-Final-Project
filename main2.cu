...
__host__ void test_run(){
    ...
    unsigned char* rgb_image = stbi_load("zelda.png", &width, &height, &channel, 3); // 3 means RGB
    int N = width * height;
    int threads_per_block = 1024;
    // int num_block = (N + threads_per_block-1)/threads_per_block;
    int num_block = ((width + 32 - 1) / 32) * ((height + 32 - 1) / 32);
    ...
    float* dCdf;
    // cudaMallocManaged((void **) &dCenterCdf, num_block * sizeof(float));
    // cudaMemset(dCenterCdf, 0.0, num_block * sizeof(float));
    ...
    stbi_write_png("zelda2.png", width, height, channel, rgb_image, width*3);
